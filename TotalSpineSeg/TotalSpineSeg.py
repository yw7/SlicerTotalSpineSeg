...
class TotalSpineSeg(ScriptedLoadableModule):
    ...
    def __init__(self, parent):
        ...
        self.parent.title = _("Total Spine Seg")
        self.parent.categories = [translate("qSlicerAbstractCoreModule", "Segmentation")]
        self.parent.dependencies = []
        self.parent.contributors = ["Yihao Wang"]
        self.parent.helpText = _("""
3D Slicer extension for fully automatic total spine instance segmentation using "TotalSpineSeg" AI model.
See more information in the <a href="https://github.com/yw7/SlicerTotalSpineSeg">extension documentation</a>.
""")
        self.parent.acknowledgementText = _("""
This module uses <a href="https://github.com/neuropoly/totalspineseg">TotalSpineSeg</a> AI model.
If you use TotalSpineSeg in your research, please cite:
TotalSpineSeg: Hierarchical Instance Segmentation of the Spine in MR images.
...
class TotalSpineSegWidget(ScriptedLoadableModuleWidget, VTKObservationMixin):
    ...
    def onSetLicense(self):
        import qt
        licenseText = qt.QInputDialog.getText(slicer.util.mainWindow(), _("Set TotalSpineSeg license key"), _("License key:"))

        success = False
        with slicer.util.tryWithErrorDisplay(_("Failed to set TotalSpineSeg license."), waitCursor=True):
            if not licenseText:
        ...
        if success:
            slicer.util.infoDisplay(_("License key is set. You can now use TotalSpineSeg tasks that require a license."))
...
class TotalSpineSegLogic(ScriptedLoadableModuleLogic):
    ...
    def __init__(self):
        ...
        if sys.version_info < (3, 12):
            ...
            self.totalSpineSegPythonPackageDownloadUrl = "https://github.com/neuropoly/totalspineseg/archive/main.zip"
        else:
        ...
        self.loadTotalSpineSegLabelTerminology()

    def loadTotalSpineSegLabelTerminology(self):
        """Load label terminology from totalsegmentator_snomed_mapping.csv file.
        Terminology entries are either in DICOM or TotalSpineSeg "Segmentation category and type".
        """
        ...
        with open(totalSpineSegTerminologyMappingFilePath, "r") as f:
            ...
            for row in reader:
                ...
                # Determine segmentation category (DICOM or TotalSpineSeg)
    ...
    def setupPythonRequirements(self, upgrade=False):
        ...
        # Install TotalSpineSeg with selected dependencies only
    ...
    def processVolume(self, inputFile, inputVolume, outputSegmentationFolder, outputSegmentation, outputSegmentationFile, task, subset, cpu, totalSpineSegCommand, fast):
        ...
        # Launch TotalSpineSeg in fast mode to get initial segmentation, if needed

        #options.extend(["--nr_thr_saving", "1"])
        #options.append("--force_split")

        # Launch TotalSpineSeg
    ...
    def setTerminology(self, segmentation, segmentName, segmentId):
        ...
        if segmentName in self.totalSpineSegLabelTerminology:
            ...
            try:
                ...
                if not TotalSpineSegLogic.isDefaultColor(color):
...
class TotalSpineSegTest(ScriptedLoadableModuleTest):
    ...
    def test_TotalSpineSegSubset(self):
        """ Ideally you should have several levels of tests.  At the lowest level
        tests should exercise the functionality of the logic with different inputs
        (both valid and invalid).  At higher levels your tests should emulate the
        way the user would interact with your code and confirm that it still works
